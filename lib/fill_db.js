...
import { createFolderForUser } from "../model/folder.js";
/**
...
async function main() {
  ...
  for (const user of users) {
    folders = await Promise.all([
      await createFolderForUser(user.id, "Documents"),
      await createFolderForUser(user.id, "Pictures"),
      await createFolderForUser(user.id, "Work"),
      await createFolderForUser(user.id, ""),
    ]);
    console.log(folders);
  }
}
